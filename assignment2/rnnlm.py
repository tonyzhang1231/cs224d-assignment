...
class RNNLM(NNBase):
    ...
    def __init__(self, L0, U0=None,
        ...
        self.sparams.L = L0.copy()  # This is a PackedVector object, but self.sgrads.L is a SparseDelta object
    ...
    def _acc_grads(self, xs, ys):
        ...
        # predicted probas
        ps = zeros((ns, self.vdim))  # y_hat in the handouts
        ...
        for t in xrange(ns):
            hs[t] = sigmoid(self.params.H.dot(hs[t-1]) + self.sparams.L[xs[t]])  # h(t) = sigmoid(H * h(t-1) + L[x(t)])
            ps[t] = softmax(self.params.U.dot(hs[t]))                   # y(t) = softmax(U * h(t))

        ##
        # Backward propagation through time
        for t in range(ns)[::-1]:   # at time point t
            delta = ps[t] - make_onehot(ys[t], self.vdim) # yhat - y
            self.grads.U += outer(delta, hs[t])

            delta_t = self.params.U.T.dot(delta) * hs[t] * (1 - hs[t])
            for k in xrange(self.bptt):
                if t - k - 1 > -2:
                    self.grads.H += outer(delta_t, hs[t - k - 1])
                    self.sgrads.L[xs[t - k]] = delta_t
                    delta_t = dot(self.params.H.T, delta_t) * hs[t - k - 1] * (1 - hs[t - k - 1])  # update delta_t
