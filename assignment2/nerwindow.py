...
class WindowMLP(NNBase):
    ...
    def _acc_grads(self, window, label):
        ...
        h = tanh(a)
        z = self.params.U.dot(h) + self.params.b2
        p = softmax(z)
        ##
        # Backpropagation
        ...
        grad_a = self.params.U.T.dot(delta) * (1 - h ** 2)

        self.grads.W += outer(grad_a, x) + self.lreg * self.params.W
        self.grads.b1 += grad_a

        dL = self.params.W.T.dot(grad_a).reshape(self.window_size, self.word_vec_size)
    ...
    def compute_loss(self, windows, labels):
        ...
        J = 0
        for i in xrange(len(windows)):   # Is it possible to use fully-vectorized method instead of for loop?
            x = hstack(self.sparams.L[windows[i], :])  # the same as above 
            h = tanh(self.params.W.dot(x) + self.params.b1)
            p = softmax(self.params.U.dot(h) + self.params.b2)
